...
if [ "$health" == "OK" ]; then
                ...
                echo "WARNING - CPU working, but needs attention!"
                exit 1

elif [ "$health" == "Critical" ]; then
                echo "CRITICAL - CPU not working correctly and requires immediate attention!"
                exit 2
else
                echo "UNKNOWN - Plugin was unable to determine the status for the CPU!"
                exit 3
fi
